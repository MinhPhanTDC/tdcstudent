...
export interface SessionsDropdownProps {
  /** Current value of completed sessions */
  value: number;
  /** Maximum allowed sessions (requiredSessions) */
  maxValue: number;
...
/**
 * Inline dropdown for editing session count
...
export function SessionsDropdown({
  ...
  error = null,
  className,
}: SessionsDropdownProps): JSX.Element {
    ...
    >
      <div className="flex flex-col gap-2">
        ...
        <select
          ...
          aria-label="Chọn số buổi hoàn thành"
          aria-invalid={!!error}
        >
          {options.map((opt) => (
        ...
        {/* Error message - Requirements: 2.4 */}
        {error && (
